...
module.exports.authorize_get= function(req, res, next){
    ...
    return res.redirect(`https://accounts.google.com/o/oauth2/v2/auth?client_id=215712698920-u9s91jupvpol0pqt68mjk2n5q7slguqa.apps.googleusercontent.com&redirect_uri=http://dev.tegoesports.com/google/callback&scope=https://www.googleapis.com/auth/admin.directory.customer.readonly&response_type=code&state=${state}`);
}
    ...
    if(crypto.createHash('md5').update(req.cookies['connect.sid']).digest("hex") === req.query.state){
        ...
        request({
            ...
            form: {
                ...
                'code': code,
                'redirect_uri': 'http://dev.tegoesports.com/google/callback'
            }
          }, function(error, res) {
                if(error){
                  response.redirect('/');
